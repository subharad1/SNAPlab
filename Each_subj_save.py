...
for k, bdf in enumerate(bdfs):
    ...
    if subj == 'HR' or subj == 'SP004' or subj == 'HL004' or subj == 'HL010':
        if k == 0:
    ...
    if events_cues_use:
        epochs_cue = mne.Epochs(raw, eves, event_id=events_cues_use, baseline=(-0.5, 0), proj=True,
...
im,_ = mne.viz.plot_topomap(norm_avg_power_alpha_left_right_pre, info_picked, names=channels,  axes=ax, ch_type='eeg',show=False)
...
fig, ax = plt.subplots(figsize=(8, 6))
im,_ = mne.viz.plot_topomap(norm_avg_power_alpha_left_right_dur, info_picked, names=channels,  axes=ax, ch_type='eeg',show=False)
plt.colorbar(im, ax=ax)
plt.show()
